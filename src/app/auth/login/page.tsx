    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
          <h1 className="text-2xl font-display text-navy mb-1 tracking-wide">SIGN IN</h1>
          <p className="text-sm text-mid mb-6">Welcome back to the community.</p>
          ...
          <form onSubmit={handleCredentials} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPw ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" className={inputClass + ' pr-10'} />
                <button type="button" onClick={() => setShowPw(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-mid hover:text-navy">
                  {showPw ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
              </div>
            </div>
            <button type="submit" disabled={isPending} className="w-full py-3 rounded-lg bg-orange text-white font-semibold text-sm hover:bg-orange-dark disabled:opacity-50 transition-colors">
          ...
          <button onClick={() => signIn('google', { callbackUrl })} className="w-full py-3 rounded-lg border border-gray-200 text-sm font-medium text-navy hover:bg-gray-50 flex items-center justify-center gap-3 transition-colors">
            {GOOGLE_SVG} Continue with Google
          </button>

          <p className="text-center text-sm text-mid mt-6">
            New to Splinters?{' '}
            <Link href="/auth/register" className="text-orange hover:underline font-medium">Join free</Link>
          </p>
        </div>
      </div>
    </div>
